...
sap.ui.define([
    ...
    ClientSideTargetResolution.prototype._mixAppStateIntoResolutionResultAndRename = function (oMatchingTarget, oAppStateService) {
        ...
        function cleanupAndResolve (oDeferred, oTarget) {
        ...
        var oSignature = oMatchingTarget.inbound && (oMatchingTarget.inbound.signature || {});
        var bChangeExistingAppState = hasExtendedUserDefaults(oResolutionResult)
            || oSignature.additionalParameters === "ignored"
            || that._hasRenameTo(oMatchingTarget);
        ...
        if (sSourceAppStateKey) {
            ...
            // are immutable)
            oAppStateService.getAppState(sSourceAppStateKey)
                .done(function (oSourceAppState) {
                    var oSourceAppStateData = oSourceAppState.getData();
                    ...
                    var bIgnoreAdditionalParameters = oSignature.additionalParameters === "ignored";
                    if (!hasExtendedUserDefaults(oResolutionResult) && !that._hasRenameTo(oMatchingTarget) && !bIgnoreAdditionalParameters) {
                        // keep old AppState
                        cleanupAndResolve(oDeferred, oMatchingTarget);
                    } else {
        ...
        if (hasExtendedUserDefaults(oResolutionResult)) {
            ...
            fnMergeAppState(oEmptyAppState);
            return oDeferred.promise();
        }
