...
/*global jQuery, sap*/
    ...
    AnchorNavigationBar.prototype._scrollToGroupByGroupIndex = function (groupIndex, speed) {
        var anchorBar = Device.system.tablet ? jQuery(".sapUshellAnchorNavigationBarItemsScroll")
                        : jQuery(".sapUshellAnchorNavigationBarItems"),
        ...
        if (jsSelectedItem) {
            ...
            // Since the offsets of the elements can get large negative values,
            // but scrollLeft will be always set to 0 in case of negative value
            // In order to calculate the relative scroll position of item inside the anchor bar, we calculate here the overall
            // Width of the anchor items and then add the selected group offset to it,
            // by that preventing the scroll value from getting negative.

            // add 2rem space + 1rem padding for the left overflow arrow
            scrollValueX = this.bIsRtl ? this._normalizeScrollBarWidth() + selectedItemOffset + 200 : selectedItemOffset - anchorBarOffset;
    ...
    AnchorNavigationBar.prototype.isMostLeftAnchorItemVisible = function () {
        var jqNavigationBar = jQuery(".sapUshellAnchorNavigationBar"),
            navigationBarOffsetLeft = !jQuery.isEmptyObject(jqNavigationBar)
