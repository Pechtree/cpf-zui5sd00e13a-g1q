...
sap.ui.define( [
    ...
    oReadCatalog, // TODO should be loaded lazy only when really needed
    oReadVisualization,
    GenericTile,
    oMobileLibrary,
    ComponentContainer,
     ...
     function LaunchPageAdapter (oUnused, sParameter, oAdapterConfiguration) {
        ...
        this._getTileForUrl = function (oTile) {
            var oTileComponentLoadInfo = {
                componentName: oTile.indicatorDataSource ? sDynamicTileComponent : sStaticTileComponent
            };
            return {
        ...
        /**
         * Calculates data and target app for a given group tile using its visualisation.
         ...
         *  to the fail handler which will be called in this case.
         */
        this._resolveTileByVizId = function (oTile, oSite) {
            ...
            if (!sVisualizationId) {
                return reject(oLogLevel.ERROR,
                    ["Cannot resolve tile '", oTile.id, "': vizId must be specified"].join(""));
            }

            oVisualization = oReadVisualization.get(oSite, sVisualizationId);
            if (!oVisualization) {
                return reject(oLogLevel.ERROR,
                    ["Cannot resolve tile '", oTile.id, "': no visualization found for vizId '", sVisualizationId, "'"].join(""));
            }
            ...
            if (!oVisualizationType) {
                return reject(oLogLevel.ERROR,
                    ["Cannot resolve tile '", oTile.id, "': no visualization type found for vizTypeId '", sVisualizationTypeId, "'"].join(""));
            }
            ...
            if (sAppId) {
                ...
                if (!oAppDescriptor) {
                    return reject(oLogLevel.INFO,
                        ["Tile '", oTile.id, "' filtered from result: no app descriptor found for appId '",
                ...
                if (oConfig==undefined) {
                    return reject(oLogLevel.INFO,
                        ["Tile '", oTile.id,
                        "' filtered from result: Neither target app nor configuration given in visualization '",
                        sVisualizationId, "'"].join(""));
                }
         ...
         */
        this._getFirstInbound = function (oAppDescriptor, sId) {
         ...
         *  item type
         */
        this.getTileType = function (oItem) {
         ...
         *  A group card object
         * @returns {jQuery.Deferred}
         *  A promise that resolves to an object which is the manifest of the given card.
         */
        this.getCardManifest = function (oGroupCard) {
            ...
            oDeferred.resolve(oResolutionResult.tileComponentLoadInfo);
    ...
    /**
        ...
        this._createTileComponentProperties = function (oTileComponentData, oTileComponentLoadInfo) {
            ...
            if (jQuery.isEmptyObject(oTileComponentLoadInfo)) {
