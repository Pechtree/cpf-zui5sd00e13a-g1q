...
sap.ui.define([
    ...
    function _handleHeaderResize(oParams) {
        ...
        handleHomeAndBackButtonsVisibility();
     ...
     */
    function handleHomeAndBackButtonsVisibility () {
        if (Config.last("/core/shell/enableFiori3")) {
            // There is no home button in Fiori3 header
            // Back button is always visible or isn't rendered
            return;
        }

        var isLSizeWidthDocking = jQuery("#mainShell").width() < 1024
            && jQuery(".sapUshellContainerDocked").length > 0,
    ...
    function handleEndItemsOverflow (oParams) {
        ...
        } else if (aEndItems.length < 2 || (aEndItems.length === 2 && aEndItems.indexOf("NotificationsCountButton") !== -1)) {
            // If there are up to 2 items and one of them is Notifications, don't show the overflow button
            return;
        }
    ...
    function _createInitialState (oConfig) {
        ...
        if (Config.last("/core/shell/enableFiori3")) {
            var aBaseStates = Object.keys(BASE_STATES);
            for (var i = 0; i < aBaseStates.length; i++) {
                var aHeadItems = BASE_STATES[aBaseStates[i]].headItems || [];
                var nMeAreaButtonIndex = aHeadItems.indexOf("meAreaHeaderButton");

                if (nMeAreaButtonIndex > -1) {
                    aHeadItems.splice(nMeAreaButtonIndex, 1);
                }
            }
        }
    ...
    return {
        ...
        handleHomeAndBackButtonsVisibility: handleHomeAndBackButtonsVisibility,
