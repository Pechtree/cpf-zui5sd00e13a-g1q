 ...
 * @private
 */
sap.ui.define(["sap/ushell/Config"], function (Config) {
	"use strict";
...
sap.ui.define([
    ...
    "sap/ushell/Config"
], function (AccessibilityCustomData, Config) {
    "use strict";
    ...
    ComponentKeysHandler.prototype = {
        ...
        _getNextTile: function (sDirection, bMoveTile, oGivenInfo) {
            ...
            if (oNextGroup) {
                var aNextGroupTiles = oNextGroup.getTiles(),
        ...
        setTileFocus: function (jqTile) {
            ...
            // remove tablindex from all cards
            jQuery(".sapFCard [tabindex = 0]").get().forEach(function (oHtmlCard) {
                jQuery(oHtmlCard).attr("tabindex", -1);
            });

            // remove tablindex from all links
            jQuery(".sapMGTLineMode [tabindex = 0]").get().forEach(function (oHtmlLink) {
                jQuery(oHtmlLink).attr("tabindex", -1);
            });
            jQuery(".sapUshellLinkTile [tabindex = 0]").get().forEach(function (oHtmlLink) {
                jQuery(oHtmlLink).attr("tabindex", -1);
            });
        ...
        _moveScrollDashboard: function ($TileSelected) {
            ...
            var iY = -1 * (document.getElementById('dashboardGroups').getBoundingClientRect().top)
                + $TileSelected.closest(".sapUshellTileContainer")[0].getBoundingClientRect().top + 48; // TileContainerHeader offset
        ...
        _getFocusOnTile: function (jqFocused) {
            ...
            ['.sapUshellTile', '.sapFCard', '.sapUshellLinkTile'].forEach(function (sClassName) {
...
//                    sap.ushell.Container.getRenderer('fiori2').showRightFloatingContainer(false);
            ...
            var bIsNoViewports = Config.last("/core/shell/enableFiori3");
         ...
         * and not core-min
         */
        _enableAnchorBarOverflowAndCreateFooter: function () {
            if (this.oDoneBtn) {
                return;
            }
        ...
        onAfterNavigate: function (oEvent) {
            ...
            if (editHomePageBtn && editHomePageBtn.data && editHomePageBtn.data("isShellHeader")) {
