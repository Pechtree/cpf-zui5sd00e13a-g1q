...
sap.ui.define([
        ...
        return UIComponent.extend("sap.ushell.components.shell.Notifications.Component", {
            ...
            createContent: function () {
                ...
                this.subscribeAndPublishNotificationEvents();
                ...
                oNotificationToggle.addEventDelegate({
                    onAfterRendering: function () {
                        this.$().attr("aria-pressed", that.oRenderer.getNotificationsSelected());
                        if (this.getDisplayFloatingNumber() > 0) {
                            this.$().attr("aria-label", resources.i18n.getText("NotificationToggleButtonCollapsed", this.getDisplayFloatingNumber()));
                        } else {
                ...
                this.oRenderer.addShellDanglingControl(oNotificationToggle); // ??? ControlManager ???
                ...
                if (Device.media.getCurrentRange(Device.media.RANGESETS.SAP_STANDARD).name === "Phone") {
                    sap.ushell.Container.getRenderer("fiori2").getShellController().handleEndItemsOverflow({
                        name: 'Phone',
            ...
            _notificationsUpdateCallback: function () {
                ...
                sap.ushell.Container.getService("Notifications").getNotifications().done(function (aNotifications) {
                    ...
                    sap.ushell.Container.getService("Notifications")._initializeDismissNotifications();
            ...
            _handleNotificationsPreviewVisibility: function (bEnableNotificationsPreview) {
                ...
                sap.ui.getCore().getEventBus().publish("shell", "setPlaceForNotificationsPreview", {EnableNotificationsPreview: bEnableNotificationsPreview});
            ...
            notificationsCountUpdateCallback: function (oDependencyPromise) {
                var that = this,
                    ...
                    bIsRightCenterViewPort = sViewPort === "RightCenter";
            ...
            /**
             * Subscribes to and publishes events for Notification Preview used to populate DOM
             * and listens to changes in Notification Preview
             */
            subscribeAndPublishNotificationEvents: function () {
                var bNotificationSettingsPreviewVisible = sap.ushell.Container.getService("Notifications")._getNotificationSettingsPreviewVisible();
                if (bNotificationSettingsPreviewVisible) {
                    sap.ui.getCore()
                        .getEventBus()
                        .subscribe("sap.ushell.services.Notifications", "enablePreviewNotificationChanged", this._updateNotificationConfiguration, this)
                        .publish("shell", "notificationsPreviewContainerCreated", {
        ...
        function fnHandleToggleNotificationsView (oSource, bShow) {
            ...
            if (bShow) {
                ...
                // toggle back to preview requires to filter out items that were dismissed in view
                aRecentNotificationsArrayFiltered =
                    sap.ushell.Container.getService("Notifications").filterDismisssedItems(
                        this.oNotificationsPreviewModel.getProperty("/previewNotificationItems"),
                        sap.ushell.Container.getService("Notifications")._getDismissNotifications()
                    );
                this.oNotificationsPreviewModel
                    .setProperty("/previewNotificationItems", aRecentNotificationsArrayFiltered);
    ...
    function (NotificationListItem, BusyIndicator, Button, NotificationListGroup, List, IconTabFilter, IconTabBar, Text, VBox, CustomListItem) {
        ...
        sap.ui.jsview("sap.ushell.components.shell.Notifications.Notifications", {
            createContent: function (oController) {
                ...
                this.oNotificationsListDate.addEventDelegate({
                    onAfterRendering: function () {
                ...
                this.oNotificationsListPriority.addEventDelegate({
                    onAfterRendering: function () {
                        oController.handleEmptyList();
                 ...
                 *
                 * @param {string} path path of items that will required from the server if needed
                 */
                ...
                this.triggerRetrieveMoreDataForExpandedGroup = function () {
                    if (!this.getModel().getProperty("/notificationsByTypeDescending/inUpdate")) {
                        var aGroupHeaders = this.getModel().getProperty("/notificationsByTypeDescending"),
                ...
                this.oNotificationsListType.addEventDelegate({
                    onAfterRendering: function () {
        ...
        */
        ...
        _handleSwitchFlagsDataInitialization : function (oModel) {
            var oSwitchBarDataPromise = sap.ushell.Container.getService("Notifications").getUserSettingsFlags(),
                ...
                bPreviewNotificationVisible = sap.ushell.Container.getService("Notifications")._getNotificationSettingsPreviewVisible();
            ...
            oSwitchBarDataPromise.done(function (oSwitchBarData) {
                ...
                oModel.setProperty("/flags/previewNotificationVisible", bPreviewNotificationVisible );
