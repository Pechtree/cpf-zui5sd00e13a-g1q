        ...
        */
        ...
        loadGroupsFromArray: function (aGroups) {
            ...
            this.oPageBuilderService.getDefaultGroup().done(function (oDefaultGroup) {
                ...
                for (i = 0; i < that.aGroupsFrame.length; i++) {
                ...
                if (that.oDashboardView) { //Homepage start
                    oEventHub.once("firstSegmentCompleteLoaded")
                        ...
                        .do(that.handleFirstSegmentLoaded.bind(that));
        ...
        _getGroupModel: function (oGroup, bDefault, bLast, oData) {
            ...
            for (i = 0; i < aGroupTiles.length; ++i) {
                ...
                if (sTileType === "tile" || sTileType === "card") {
                    aModelTiles.push(this._getTileModel(aGroupTiles[i], bIsGroupLocked, sTileType));
                } else if (sTileType === "link") {
                    aModelLinks.push(this._getTileModel(aGroupTiles[i], bIsGroupLocked, sTileType));
                } else {
        ...
        getTileView: function (tileOrCard, groupPosition) {
            var oPageBuilderService = this.oPageBuilderService,
                sType = oPageBuilderService.getTileType(tileOrCard.object);
            ...
            oPageBuilderService.setTileVisible(tileOrCard.object, false);
            if (sType === "card") {
        ...
        _loadCardData: function (card) {
            var oCard = sap.ui.getCore().byId(card.controlId),
                sPerformanceHash = jQuery.sap.flpmeasure.startFunc(0, "Loading of data and rendering for all cards", 5, "loadCardData", card.object);
        ...
        _loadTileData: function (tile, groupPosition, tileType) {
            var oPageBuilderService = this.oPageBuilderService,
                ...
                sPerformanceHash = jQuery.sap.flpmeasure.startFunc(0, "Service + model binding and rendering for all tile-Views", 5, "getTileView", tile.object);

            oDfd = oPageBuilderService.getTileView(tile.object);
            ...
            oDfd.done(function (oView) {
                ...
                if (that.isBlindLoading()) {
                    /*
                     ...
                     therefore we read and set the size again
                     */
                    var sSize = that.oPageBuilderService.getTileSize(tile.object);
                    var bLong = sSize === "1x2";
                    if (tile.long !== bLong) {
                        tile.long = bLong;
                    }
            ...
            oDfd.fail(function () {
                if (that.sTileType === "link" && this.bLinkPersonalizationSupported) {
                    // In case call is synchronise we set the view with 'TileState' control with 'Failed' status
                    var vSubHeader = that.oPageBuilderService.getCatalogTilePreviewSubtitle(tile);
                    var vHeader = that.oPageBuilderService.getCatalogTilePreviewTitle(tile);
            ...
            if (!oTileView) {
                if (oPageBuilderService.getTileType(tile.object) === "link") {
        ...
        _getTileModel: function (oTile, isGroupLocked, sTileType) {
            ...
            if (sTileType === "card") {
                ...
                srvc.getCardManifest(oTile).done(function (oManifest) {
        ...
        loadPersonalizedGroups: function () {
            ...
            return this.oPageBuilderService.getGroups().done(function (aGroups) {
