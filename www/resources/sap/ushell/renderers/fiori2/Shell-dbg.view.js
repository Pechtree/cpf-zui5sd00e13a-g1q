...
sap.ui.define([
    ...
    sap.ui.jsview("sap.ushell.renderers.fiori2.Shell", {
        ...
        createContent: function (oController) {
            ...
            //handle open catalog
            //For fiori3 appfinder button is created by MeArea component, because this button is belong to the MeArea
            if (oConfig.enablePersonalization !== false && !Config.last("/core/shell/enableFiori3")) {
                var oHash;
                ...
                if (!oConfig.moveAppFinderActionToShellHeader && !this.oOpenCatalogItem) {
                    ...
                    this.oOpenCatalogItem = new ActionItem(id, oOpenCatalogConfig);
                    if (bHelpEnabled) {
                        this.oOpenCatalogItem.addStyleClass('help-id-openCatalogActionItem');// xRay help ID
                    }
                    this.addDanglingControl(this.oOpenCatalogItem);
                }
            }

            var oUnifiedShell;
            if (Config.last("/core/shell/enableFiori3") === true) {
                oUnifiedShell = sap.ui.xmlfragment("sap.ushell.ui.fiori3.ShellLayout", oController);
            } else {
        ...
        createShellHeader: function (oConfig, oShellModel) {
            ...
            if (Config.last("/core/shell/enableFiori3") === true) {
                ...
                oShellHeader = sap.ui.xmlfragment("sap.ushell.renderers.fiori2.ShellHeader", oHeaderController);
            }
