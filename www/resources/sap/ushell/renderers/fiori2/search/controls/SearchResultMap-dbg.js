...
sap.ui.define([
    ...
    var getDemoUrlPairsForMapTiles = function (oControl) {
        var res = null;
        //try get map tile URLs from config or return null
        if (oControl.getModel() && oControl.getModel().config && oControl.getModel().config.mapsAppId) {
            var stub1 = "https://1.base.maps.cit.api.here.com/maptile/2.1/maptile/newest/normal.day/{LOD}/{X}/{Y}/256/png8?";
            var stub2 = "https://2.base.maps.cit.api.here.com/maptile/2.1/maptile/newest/normal.day/{LOD}/{X}/{Y}/256/png8?";
            var appIdSegment, appCodeSegment, insertParamsString, url1, url2;
            appIdSegment = 'app_id=' + oControl.getModel().config.mapsAppId;
            appCodeSegment = 'app_code=' + oControl.getModel().config.mapsAppCode;
            appIdSegment = appIdSegment.replace('mapsAppId', 'app_id');
            appCodeSegment = appCodeSegment.replace('mapsAppCode', 'app_code');
            insertParamsString = "?" + appIdSegment + "&" + appCodeSegment;
            url1 = stub1.replace("?", insertParamsString);
            url2 = stub2.replace("?", insertParamsString);
            res = [
                [url1, url2]
            ];
        }
    ...
    var getUrlForMapTiles = function (urlIndex) {
        var fullUrl = null;
        var URLPairsArray = []; //can be filled either if URL contains app_id and app_code params OR param &sap-ushell-config-url=/sap/ushell_config/flpconfig.json
        try {
            URLPairsArray = sap.ushell.renderers.fiori2.search.getModelSingleton().config.mapTileLocation.URLPairs;
        } catch (e) {
            //do nothing now - at render time when model is available try getDemoUrlPairsForMapTiles(); //may return null
        }
        if (mapClientIsAvailable) {
            fullUrl = "/sap/hana/spatial/mapClient/map.xsjs?col={X}&row={Y}&level={LOD}";
        } else if (URLPairsArray && URLPairsArray.length > 0) { //take first in pairs-list
            if (urlIndex === 0) {
    ...
    return sap.ui.core.Control.extend('sap.ushell.renderers.fiori2.search.controls.SearchResultMap', {
        ...
        ifMissingTilesCheckUrlParams: function (oControl) {
            var URLPairsArray = getDemoUrlPairsForMapTiles(oControl);
            if (URLPairsArray && URLPairsArray.length > 0) {
                hasValidTileUrls = true;
                var oMapsConfig = oControl.myMap.getMapConfiguration();
                oMapsConfig.MapProvider[0].Source[0].url = URLPairsArray[0][0];
                oMapsConfig.MapProvider[0].Source[1].url = URLPairsArray[0][1];
                oControl.myMap.setMapConfiguration(oMapsConfig);
            }
        },
        renderer: function (oRm, oControl) {
            oControl.loadObjects(oControl);
            if (!hasValidTileUrls) {
                oControl.ifMissingTilesCheckUrlParams(oControl);
            }
            oRm.write('<div ');
        ...
        calculateZoomLevel: function (iScreenWidth, iKm) {
            ...
            Log.debug('zoomLevel calc: ' + zoomLevel);
            return zoomLevel;
        },
        ...
        centerMap: function () {
            ...
            Log.debug("centerMap() returns: centerLat, centerLon: " + that.centerLat + ";" + that.centerLon);
            //Log.debug("NB center of Germany: 51.126586;10.472796");
        ...
        zoomChanged: function (event) {
            ...
            //Log.debug('zoomChanged viewport', JSON.stringify(viewport), ", zoomLevel: " + zoomLevel);
            Log.debug('zoomLevel ', zoomLevel, 'LatLonDiff:', this.getLatLonDiff(viewport));
            //Log.debug("number of locations: " + event.getSource().getModel().oData.results.length);
        ...
        centerChanged: function (event) {
            ...
            //Log.debug('centerChanged viewport', JSON.stringify(viewport), ", centerPoint: " + centerPoint, ", zoomLevel: " + //zoomLevel);
            //Log.debug('centerChanged LatLonDiff:', this.getLatLonDiff(viewport));
            //Log.debug("number of locations: " + event.getSource().getModel().oData.results.length);
        ...
        setZoomLevelAfterRendering: function () {
            ...
            //iKm = Math.floor(iKm);
            //Log.debug("BOX minLat, minLon, maxLat, maxLon: ", this.minLat, this.minLon, this.maxLat, this.maxLon);
            //Log.debug('iKm for zoomLevel calc: ' + iKm);
            Log.debug('iScreenWidth for zoomLevel calc: ' + iScreenWidth);
