    ...
    return core.defineClass({
        ...
        parseItem: function (itemData) {
            ...
            for (var attributeName in itemDataStructured) {
                ...
                var attrValueFormatted;
                var attrWhyFound;
                if (util.getUrlParameter('resultValueFormatted') === "true") {
                    attrValueFormatted = undefined;
                    attrWhyFound = undefined;
                    // undefined is allowed!
                } else {
                    attrValueFormatted = '';
                    attrWhyFound = '';
                    if (metadata.type === AttributeType.Timestamp) {
                        var ui5Format = sap.ui.core.format.DateFormat.getDateTimeInstance();
                        attrValueFormatted = ui5Format.format(attrValue);
                    } else {
                        attrValueFormatted = attrValue !== undefined ? attrValue.toString() : '';
                    }
                }
            ...
            for (var restWhyfoundAttribute in whyFounds) {
                if (whyFounds[restWhyfoundAttribute][0]) {
                    ...
                    attribute = this.sina._createSearchResultSetItemAttribute({
                        ...
                        value: null,
                        valueFormatted: whyFounds[restWhyfoundAttribute][0],
                        valueHighlighted: whyFounds[restWhyfoundAttribute][0],
                        isHighlighted: true,
                        metadata: metadata
                    });
                    detailAttributes.push(attribute);
