sap.ui.define([
	...
	TAccount.prototype.onAfterRendering = function () {
		...
		if (oParent && this._hasGroupParent(oParent) && !oParent._bRendered) {
		...
		if (this._isInGroup()) {
			...
			$this.draggable({
				...
				stop: function () {
					...
					oParent.$().find(".sapSuiteUiCommonsAccountHeader").css("cursor", "pointer");
				}
			});
	...
	TAccount.prototype._getDownArrow = function () {
		if (!this._oArrowDown) {
			this._oArrowDown = new Icon({
				alt: this._getExpandAltText(true),
				decorative: false,
	...
	TAccount.prototype._setupTooltips = function () {
		var fnCheckTooltip = function (sClass) {
			...
			if (oElement[0].clientHeight < oElement[0].scrollHeight) {
	...
	TAccount.prototype.setCollapsed = function (bValue) {
		...
		this._getDownArrow().$().attr("aria-label", this._getExpandAltText(!bValue));
