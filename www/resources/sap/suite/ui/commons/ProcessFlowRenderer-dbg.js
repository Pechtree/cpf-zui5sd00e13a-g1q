...
sap.ui.define([
...
], function (jQuery, ProcessFlowLaneHeader, ProcessFlowZoomLevel, Device, encodeXML) {
	"use strict";
	...
	ProcessFlowRenderer._renderNode = function (oRm, oControl, node, isTDTagOpen) {
		if (isTDTagOpen) {
			...
			if (node.getAriaProperties()) {
				node.getAriaProperties().addAriaProperties(oRm,oAriaDefaultProperties);
			} else {
	...
	ProcessFlowRenderer._renderBasicStructure = function (oRm, oControl) {
		...
		if (oControl.getAriaProperties()) {
			oControl.getAriaProperties().addAriaProperties(oRm,oAriaDefaultProperties);
		} else {
			oRm.writeAttributeEscaped("aria-label", oControl._getAriaText());
		}
