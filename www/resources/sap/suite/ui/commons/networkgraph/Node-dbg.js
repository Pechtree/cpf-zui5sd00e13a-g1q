...
sap.ui.define([
	...
	Node.prototype._renderHtmlAttributes = function (mOptions) {
		...
		if (aAttributes.length > 0) {
			var sHtml = "<div class=\"sapSuiteUiCommonsNetworkGraphDivNodeAttributes\">";

			aAttributes.forEach(function (oAttribute) {
				var sId = oAttribute.getId();
				sHtml += "<div id=\"" + sId + "\" class=\"sapSuiteUiCommonsNetworkGraphDivNodeAttributesRow\">";

				sHtml = this._processRenderManager(mOptions.renderManager, sHtml);
	...
	Node.prototype._renderHtml = function (mOptions) {
		...
		mOptions.renderManager.write(this._renderHtmlElement("div", {
			width: this._isCircle() ? this._getCorrectWidth().width : "",
			top: this.getY() ? (this.getY() + "px") : "",
			left: this.getX() ? (this.getX() + "px") : "",
	...
	Node.prototype._renderHtmlContent = function (mOptions) {
		...
		if (bIsCircle) {
			...
			mOptions.renderManager.write(this.renderHtmlActionButtons());
			...
			mOptions.renderManager.write("<div  id=\"" + sId + "-focus\" class=\"sapSuiteUiCommonsNetworkDivFocus\">");
			...
			mOptions.renderManager.write("<div id=\"" + sId + "-focus\" class=\"sapSuiteUiCommonsNetworkDivFocus\"></div>");
	...
	Node.prototype._renderHtmlTitleIcon = function (sClass) {
		...
		if (oImage) {
			...
			sHtml = "<div " + sColorStyle + " class=\"" + sClass + "\">" + this._renderHtmlIcon(sIcon) + "</div>";
		}
	...
	Node.prototype._renderHtmlBoxContent = function (mOptions) {
		...
		if (bHasAdditionalContent) {
			...
			sHtml += this._renderHtmlAttributes(mOptions);
			sHtml += "</div>";
		}
	...
	Node.prototype._setStatusColors = function (sType) {
		...
		var bUseNodeHtml = this.getParent()._isUseNodeHtml();
	 ...
	 * @param {integer} [mArguments.width] Width of the text is trimmed to
	 ...
	 * @param {integer} [mArguments.x] X coordinate that is added to the node's top left x coordinate
	 * @param {integer} [mArguments.y] Y coordinate that is added to the node's top left y coordinate
	 * @param {integer} [mArguments.lineSize] Line size for the node, if not set, the default line size is used
	 ...
	 * @param {integer} [mArguments.x] X coordinate that is added to node's top left x coordinate.
	 * @param {integer} [mArguments.y] Y coordinate that is added to node's top left y coordinate.
	 * @param {Object} [mArguments.attributes] Additional attributes rendered with the text
	 *
	 ...
	 * @public
	 */
	Node.prototype.renderHtmlActionButtons = function () {
		var sRightId = this.getId() + "-rightdivbuttons",
			sLeftId = this.getId() + "-leftdivbuttons";
