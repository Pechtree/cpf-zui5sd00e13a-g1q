...
sap.ui.define([
	 ...
	 * @param {object} [mParameters.x=middle of view port width] x position from/to which the zoom should happen
	...
	Graph.prototype._innerRender = function (mParams) {
		...
		var SIZE_OFFSET_Y = 20,
			...
			fnSetLimits = function () {
				...
				this.$divgroups.width(this._iWidth + (this._isTwoColumnsLayout() ? 25 /*END TWO COLUMNS OFFSET*/ : 0));
	...
	Graph.prototype._ensureOnScreen = function (oItem) {
		...
		iCenterX *= this._fZoomLevel;
		iCenterY *= this._fZoomLevel;
	...
	Graph.prototype._createLegend = function () {
		...
		var fnAppendLine = function (sStatus, sType) {
			...
			sHtml += encodeXML(this._oLegendLabels[sType + sStatus] ? this._oLegendLabels[sType + sStatus] : sCustomTitle);
			sHtml += "</label>";
			sHtml += "</div>";
		}.bind(this);
		...
		var fnProcessElements = function (aItems, oCollection, sClass, sTitle, sStatusType) {
			...
			if (fnHasStatus(oCollection)) {
				...
				var oStatuses = Object.keys(oCollection).reduce(function (oAcc, sKey) {
					oAcc[sKey] = this._oLegendLabels[sStatusType + sKey] ? this._oLegendLabels[sStatusType + sKey] : sKey;
					return oAcc;
				}.bind(this), {});

				Object.keys(oCollection).sort(function (a, b) {
					return oStatuses[a] > oStatuses[b];
