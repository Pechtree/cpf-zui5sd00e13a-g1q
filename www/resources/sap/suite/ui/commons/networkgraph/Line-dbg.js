...
sap.ui.define([
	...
	Line.prototype._render = function (mOptions) {
		...
		var fnRenderArrow = function () {
			var aPoints = this._getArrowPoints(),
				oArrowAttributes = {
					id: this.getId() + "-arrow",
					"class": "sapSuiteUiCommonsNetworkLineArrow",
					style: sColorStyle,
					d: "M " + aPoints[0].x + "," + aPoints[0].y +
						" L " + aPoints[1].x + "," + aPoints[1].y +
						" L " + aPoints[2].x + "," + aPoints[2].y +
						" Z"
				};

			return this._renderControl("path", oArrowAttributes);
		}.bind(this);
	 ...
	 * @private
	 */
	Line.prototype._getArrowFragmentVector = function () {
		...
		if (this.getBends().length === 0) {
			iHolyIndex = 0;
		} else if (this.getArrowPosition() === ArrowPosition.Start) {
		...
		} else if (this.getArrowPosition() === ArrowPosition.End) {
	 ...
	 * @private
	 */
	Line.prototype._getArrowPoints = function () {
		...
		var fnCalcArrowPoint = function (oArrowVertex) {
			...
			if (this.getArrowOrientation() === ArrowOrientation.ChildOf) {
				fArrowAngle += Math.PI;
			}
		...
		oFragVector = this._getArrowFragmentVector();
