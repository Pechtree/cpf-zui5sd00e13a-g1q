...
sap.ui.define([
	...
	MoveElements.completeChangeContent = function (oChange, oSpecificChangeInfo, mPropertyBag) {
		...
		aElements.some(function (oEntry, i) {
			if (oEntry && oEntry.getId && oEntry.getId().indexOf("--edit") > 0) {
				iEditIndex = i;
			}
			if (oEntry && oEntry.getId && oEntry.getId().indexOf("--delete") > 0) {
				iDeleteIndex = i;
			}
			if (oEntry && oEntry.getId && oEntry.getId().indexOf("--relatedApps") > 0) {
				iRelatedAppsIndex = i;
			}
		});
		// END
