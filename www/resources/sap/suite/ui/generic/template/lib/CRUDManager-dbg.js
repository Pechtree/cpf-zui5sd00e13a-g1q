				...
				var oRet = new Promise(function(resolve, reject) {
					...
					oServices.oApplication.getDraftSiblingPromise(oContext).then(function(oSiblingContext){
						...
						oActivationPromise.then(function(oResponse) {
							...
							// In case of draft enabled applications, with only "DraftAdministrativeData" in root expand, we skip the "read" request below.
							// This is because until the below "read" request is resolved, all other further requests(example: a table or a chart) are stalled.
							// With the change, this request for "DraftAdministrativeData" is resolved in fnRebindHeaderData(ComponentUtils).
							// And the request for "DraftAdministrativeData" goes parallel to the further requests.
							var bSkip = oComponentUtils.isDraftEnabled() && aExpand && aExpand.length === 1 && (aExpand.indexOf("DraftAdministrativeData") >= 0);
							if (aExpand && !bSkip) {
