sap.ui.define(["jquery.sap.global",
	...
	"sap/base/Log"
], function(jQuery, BaseObject, Event, MessageBox, Table, Sorter,
	testableHelper, JSONModel, coreLibrary, AnnotationHelper, modelHelper, GroupHeaderFormatter, SelectionVariant, Controller, Log) {
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getNavigateAfterCancelPromise() {
			var oContext = oController.getView().getBindingContext();
			var oSiblingPromise = oServices.oApplication.getDraftSiblingPromise(oContext);
			return oSiblingPromise.then(function(oActive){
				return oServices.oApplication.getNavigateAfterCancelPromise(oActive);
			});
		}
...
//			}
		...
		function onListNavigateImpl(oEventOrSource, oState, onListNavigationExtension, oBindingContext){
			// unfortunately, the interface of onListNavigationExtension was build with providing the original event
			// however, in some cases, we now actually have to use a different event to trigger the navigation, so we have to fake
			// the original event for the extension in a way, that the event in the extension is like the extension developer expects
			// (i.e. like it used to be)
			// different cases are:
			// - UI Table: here everything remains as before, so we can use the event as it is
			// - responsive Table: Now itemPress event of table must be used instead of press event of ColumnListItem, so we have to replace the event
			// - chart: use as before (although it seems not to make too much sense...)
			// - programmatical navigation: we have no event, but a ColumnListItem, so we need to construct the event
			var oEvent, oEventSource;
			if (oEventOrSource instanceof Event){
				...
				if (oCommonUtils.isMTable(oEventSource)){
					...
					// other tables or chart: use the given event
					oEvent = oEventOrSource;
				}
			} else {
				// programmatical case
				oEventSource = oEventOrSource;
			}
			oEvent = oEvent || new Event("press", oEventSource,  {id: oEventSource.getId()});
		...
		function onListNavigate(oEventOrSource, oState, onListNavigationExtension, oBindingContext, bIsProgrammatic) {
			if (bIsProgrammatic){
				onListNavigateImpl(oEventOrSource, oState, onListNavigationExtension, oBindingContext);
			} else {
				// This will care for:
				...
				oCommonUtils.processDataLossConfirmationIfNonDraft(onListNavigateImpl.bind(null, oEventOrSource, oState, onListNavigationExtension, oBindingContext), jQuery.noop, oState);
			}
		}
...
//			}
		...
		function getDiscardDraftPopover(fnDiscardDraft) {
			...
			var oDraftPopover = oCommonUtils.getDialogFragment("sap.suite.ui.generic.template.fragments.DiscardDraftPopover", {
				onDiscardConfirm: function() {
					Log.info("Draft cancellation confirmed");
					if (!bIsDiscardDraftPopoverActive) {
						Log.info("Draft popover no longer active -> Ignore.");
						return;
					}
		...
		function fnDiscardDraftImpl(oNavigateAfterCancelPromise, fnFinally){
			var oPromise = new Promise(function(fnResolve, fnReject){
				oNavigateAfterCancelPromise.then(function(fnNavigate) {
					Log.info("Active information for current draft has been read. Start discarding the draft");
					var oDiscardPromise = oServices.oCRUDManager.deleteEntity(false, true);
					oDiscardPromise.then(function(){
						Log.info("Draft was discarded successfully");
						fnFinally();
		...
		function onDiscardDraft(oEvent, oBeforeSaveHelper) {
			var oNavigateAfterCancelPromise = getNavigateAfterCancelPromise();
			oServices.oApplication.performAfterSideEffectExecution(function(){
				if (oServices.oApplication.getBusyHelper().isBusy()) {
					Log.info("Ignore discarding confirmation as app is already busy");
				...
				if (bIsDraftModified){ // if the user has done any changes to the draft, a confirmation popover is displayed
					...
					var fnFinally = function(){
						...
						Log.info("Draft popover closed");
					};
					oDiscardPopover = getDiscardDraftPopover(fnDiscardDraftImpl.bind(null, oNavigateAfterCancelPromise, fnFinally));
					...
					fnDiscardDraftImpl(oNavigateAfterCancelPromise, jQuery.noop);
				}
			});
		}
		...
		function fnDiscardDraftWithoutDialog(){
			if (oComponentUtils.getViewLevel() !== 1 || !oComponentUtils.isDraftEnabled()){
				Log.warning("Discard draft only possible on root level of draft enabled entities");
				return Promise.reject();
			}
			var oComponent = oController.getOwnerComponent();
			if (!(oComponentUtils.isComponentActive() && oComponent.getModel("ui").getProperty("/editable"))){
				Log.warning("Discard draft only possible in edit mode");
				return Promise.reject();
			}
			var oNavigateAfterCancelPromise = getNavigateAfterCancelPromise();
			return fnDiscardDraftImpl(oNavigateAfterCancelPromise, jQuery.noop);
		}
