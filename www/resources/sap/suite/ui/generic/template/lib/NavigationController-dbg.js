...
sap.ui.define(["jquery.sap.global",
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		function getParStringPromise(oAppStates, bAddLevel0Info){
			var oAppStatePromise = bAddLevel0Info ? fnAddUrlParameterInfoForRoute("root", oAppStates) : Promise.resolve();
			return oAppStatePromise.then(fnAppStates2ParString.bind(null, oAppStates));
		}
		...
		function fnNavigateByReplacingAppState(mAppStates){
			var sHash = oNavigationControllerProxy.oHashChanger.getHash() || "";
			sHash = sHash.split("?")[0];
			var oParStringPromise = getParStringPromise(mAppStates, false);
			return fnNavigateToParStringPromise(sHash, oParStringPromise, null,  false, null, true);
		}
		...
		oNavigationControllerProxy.navigateByReplacingAppState = fnNavigateByReplacingAppState;
