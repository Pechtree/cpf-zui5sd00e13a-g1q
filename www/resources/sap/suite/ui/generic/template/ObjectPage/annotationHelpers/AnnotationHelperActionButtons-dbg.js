...
sap.ui.define(["sap/base/Log", "sap/ui/model/odata/AnnotationHelper", "sap/suite/ui/generic/template/js/AnnotationHelper", "sap/suite/ui/generic/template/lib/testableHelper"], function(Log, AnnotationHelperModel, AnnotationHelper, testableHelper) {
	...
	function getEditActionButtonVisibility(oInterface, mRestrictions, oEntitySet, bIsDraftEnabled) {
		...
		if (bIsDraftEnabled){ // Exclude the situation that there is already a draft for this user (in this case the CONTINUE_EDITING-button would be shown)
	...
	function getCallAction(oInterface, oDataField){
		...
		return "._templateEventHandlers.onCallAction('" + sAction + "', '" + sLabel + "', '" + sInvocationGrouping + "')";
	}
