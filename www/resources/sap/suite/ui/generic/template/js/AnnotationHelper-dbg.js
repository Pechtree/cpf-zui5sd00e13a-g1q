sap.ui.define(["sap/ui/model/odata/AnnotationHelper", "sap/ui/comp/smartfield/SmartField"],
	function(AnnotationHelperModel, SmartField) {
	"use strict";
	...
	var oAnnotationHelper = {
		...
		buildDeleteButtonEnablementExpression: function (mFacet, oTabItem) {
					...
					var sButtonId = sap.suite.ui.generic.template.js.StableIdHelper.getStableId({
						...
						subType: "Delete",
						sFacet: oAnnotationHelper.getStableIdPartFromFacet(mFacet)
					});
		...
		*/
		isSingleSection: function(oAnnotatedSection, oExtensionSection, oManifest) {
			...
			for (var key in oExtensionSection) {
				...
				for (var i = 0; i < oAnnotatedSection.length; i++) {
					if (!oAnnotatedSection[i].Facets && (!oAnnotatedSection[i]["com.sap.vocabularies.UI.v1.Hidden"] || (oAnnotatedSection[i]["com.sap.vocabularies.UI.v1.Hidden"] && oAnnotationHelper.getBindingForHiddenPath(oAnnotatedSection[i])))) {
						...
						for (var j = 0; j < oAnnotatedSection[i].Facets.length; j++) {
							if ((!oAnnotatedSection[i].Facets[j]["com.sap.vocabularies.UI.v1.Hidden"] || (oAnnotatedSection[i].Facets[j]["com.sap.vocabularies.UI.v1.Hidden"] && oAnnotationHelper.getBindingForHiddenPath(oAnnotatedSection[i].Facets[j])))) {
		...
		setRowHighlight: function(entityType) {
			return "{parts: [{path: 'IsActiveEntity'}, {path: 'HasActiveEntity'}, {path: '" + entityType["com.sap.vocabularies.UI.v1.LineItem@com.sap.vocabularies.UI.v1.Criticality"].Path + "'}], formatter: 'sap.suite.ui.generic.template.js.AnnotationHelper.setInfoHighlight'}";
		},
