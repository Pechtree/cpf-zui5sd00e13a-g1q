sap.ui.define([
	...
	"sap/ui/core/library",
	"sap/base/Log"
], function (Control, KpiTagController, KpiUtil, KpiAnnotationHelper, AnnotationHelper, FilterUtil, V4Terms, ODataModelUtil, KpiTag, KpiProvider, Device, SelectionVariant, Filter, JSONModel, Locale, ResponsivePopover, MessageStrip, NumberFormat, SapMLibrary, SapCoreLibrary, Log) {
	"use strict";
	var TARGET = "Target",
		MAXIMIZE = "Maximize",
		MINIMIZE = "Minimize";
	return KpiTag.extend("sap.suite.ui.generic.template.AnalyticalListPage.control.SmartKpiTag", {
		...
		_kpiSuccessHandler: function (data, response) {
			...
			if (this._oConfig.properties.unitOfMeasure) {
				(this._oConfig.properties.unitOfMeasure.hasOwnProperty("Path")) ? this.bindProperty("unit", {
					path: "/" + this._oConfig.properties.unitOfMeasure.Path
				}) : this.setProperty("unit", this._oConfig.properties.unitOfMeasure.String || this._oConfig.properties.unitOfMeasure);
			}
		...
		onAfterRendering: function () {
			var oRb = this.getModel("i18n").getResourceBundle(),
				...
				KPITooltipKey;
			var color_kpi = this.getIndicator();
			...
			color_kpi = oRb.getText(KPITooltipKey, [(this.kpiSettings && this.kpiSettings.props ? this.kpiSettings.props.title : ""), rightTooltip]);
			...
			if (nameFromPath && nameFromPath.indexOf(">") > 0) {
				this.bindProperty("tooltip", {
					...
					formatter: function (value) {
						...
						return oRb.getText(KPITooltipKey, [value, rightTooltip]);
					}.bind(this)
				});
			} else {
				this.setTooltip(color_kpi);
			}

			// checkking if KPI is having error or warning in determining value and setting aria label accordingly.
			if (this.getError()) {
				KPITooltipKey = this.getErrorType() == "Error" ? "KPI_DETERMINING_ERROR" : "KPI_DETERMINING_WARNING";
			}
