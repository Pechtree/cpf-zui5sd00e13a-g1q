...
sap.ui.define([
	...
	var ODataControlFactory = ControlFactoryBase.extend("sap.ui.comp.smartfield.ODataControlFactory", /** @lends sap.ui.comp.smartfield.ODataControlFactory.prototype */{
		constructor: function(oModel, oParent, oMetaData) {
			...
			this.oAmountField = null;
			this.bTextInEditModeSourceValidMetadata = false;
	...
	ODataControlFactory.prototype._createEdmString = function() {
		...
		this.bTextInEditModeSourceValidMetadata = bEdmPropertyTypeSupported && this._checkTextInEditModeSourceRequiredMetadata(sTextInEditModeSource);

		if (this.bTextInEditModeSourceValidMetadata) {
			mAttributes = this.createAttributes("", this._oMetaData.property, mNames);
			mAttributes[sValuePropertyMap] = this.getTextArrangementBindingInfo();
		} else {
		...
		if (this.bTextInEditModeSourceValidMetadata && (sTextInEditModeSource === TextInEditModeSource.ValueList)) {
			var INTERNAL_NAMED_MODEL = "__sap.ui.comp.smartfield.type.TextArrangementModel",
				sDescriptionPath = mAttributes.value.parts[1].path;

			if (sDescriptionPath !== "") {
				var sValueListEntityAbsolutePath = sDescriptionPath.split("/")[1];
				oControl.setModel(this._oModel, INTERNAL_NAMED_MODEL);
				oControl.bindElement(INTERNAL_NAMED_MODEL + ">/" + sValueListEntityAbsolutePath);
			}
		}
	...
	ODataControlFactory.prototype._checkTextInEditModeSourceRequiredMetadata = function(sTextInEditModeSource) {
		switch (sTextInEditModeSource) {
			case TextInEditModeSource.None:
				return false;

			case TextInEditModeSource.NavigationProperty:
				var oNavigationPropertyMetadata = this._oMetaData.annotations.text,
				...
				return this._oHelper.checkNavigationPropertyRequiredMetadata({
					propertyName: this._oMetaData.property.property.name,
					entityType: this._oMetaData.entityType,
					entityTypeOfNavigationProperty: oEntityTypeOfNavigationProperty,
					textAnnotation: this._oMetaData.property.property["com.sap.vocabularies.Common.v1.Text"]
				});

			case TextInEditModeSource.ValueList:
				return this._oHelper.checkValueListRequiredMetadataForTextArrangment({
					propertyName: this._oMetaData.property.property.name,
					entityType: this._oMetaData.entityType,
					valueListAnnotation: this._oMetaData.property.valueListAnnotation
				});
	...
	ODataControlFactory.getTextArrangementPaths = function(sTextInEditModeSource, oMetadata) {
	...
	ODataControlFactory.prototype.getTextArrangementBindingInfo = function(mSettings) {
		var oType = mSettings && mSettings.type,
			oMetadata = this._oMetaData;

		if (!oType) {
			var oBindingInfo = this._oParent.getBindingInfo("value");
			oType = (oBindingInfo && oBindingInfo.type) || {};
			var mTextArrangementBindingPaths = ODataControlFactory.getTextArrangementPaths(this._oParent.getTextInEditModeSource(), oMetadata);

			oType = this._oTypes.getType(oMetadata.property, oType.oFormatOptions, oType.oConstraints, {
				composite: true,
		...
		return {
			...
			parts: [
				...
				{
					path: this._getTextAnnotationPropertyPath(this._oParent.getTextInEditModeSource())
				}
			]
		};
	};

	ODataControlFactory.prototype._getTextAnnotationPropertyPath = function(sTextInEditModeSource) {

		switch (sTextInEditModeSource) {
			case TextInEditModeSource.NavigationProperty:
				return this._oHelper.getTextAnnotationPropertyPath(this._oMetaData);

			case TextInEditModeSource.ValueList:
				var oBinding = this._oParent.getBinding("value"),
					oEdmProperty = this._oMetaData.property;

				// return the absolute path to the value list entity property e.g.: /VL_SH_H_CATEGORY('PR')/LTXT
				return this._oHelper.getAbsolutePropertyPathToValueListEntity({
					entitySet: oEdmProperty.valueListEntitySet,
					entityID: oBinding && oBinding.getValue(),
					property: oEdmProperty.valueListKeyProperty
				});
	...
	ODataControlFactory.prototype._createEdmUOM = function() {
		var bIsCurrency = this._oMetaData.annotations.uom && this._oHelper.oAnnotation.isCurrency(this._oMetaData.annotations.uom.property.property),
		...
		this.oAmountField = oAmountInputField;
	...
	ODataControlFactory.prototype._createEdmUOMAttributes = function(mSettings) {
		...
		var bIsCurrency = mSettings.currency || (this._oMetaData.annotations.uom && this._oHelper.oAnnotation.isCurrency(this._oMetaData.annotations.uom.property.property));
	...
	ODataControlFactory.prototype.bind = function() {
		...
		var aPropertiesNames = this._oFieldControl.getBindablePropertiesNames(),
			oConfig = this._oParent.data("configdata");

		if (oConfig && oConfig.configdata) {
			this.triggerCreationOfInnerControls(this._oMeta, aPropertiesNames);
			return Promise.resolve();
		}
		...
		if (bTextInEditModeSourceNotNone) {
			oValueListAnnotationLoadedPromise.then(function(oValueListAnnotations) {
	 ...
	 *
	 * @param {array} aPropertiesNames current bindings on <code>SmartField</code>
	 * @private
	 */
	ODataControlFactory.prototype._bind = function(aPropertiesNames) {
		var oBindingInfo,

			// make sure that no exceptions occur, if the property is not valid
			// => necessary for extensibility use cases, if an extension field has been deleted and the UI has not yet been adapted.
			// and if the smart field's value property is not bound, but a URL has to be displayed.
			oFormatters = this._oFieldControl.getControlProperties(this._oMetaData, aPropertiesNames);

		for (var sProperty in oFormatters) {
			oBindingInfo = this._oBinding.fromFormatter(this._oMetaData.model, oFormatters[sProperty]);
