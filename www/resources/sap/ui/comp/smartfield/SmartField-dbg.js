...
sap.ui.define([
	...
	SmartField.prototype.setEditableProperty = function(sPropertyName, bEditable, bSuppressInvalidate) {
		...
		var bEditableStateChanged = bEditable !== bOldEditable,
			bModelUpdate = isPropertyBeingUpdatedByModel.call(this, sPropertyName);
	...
	function isPropertyBeingUpdatedByModel(sPropertyName) {
	...
	SmartField.prototype.setUomVisible = function(bUomVisible) {
		...
		if (oUomField) {
			...
			oUomField.setVisible(bUomVisible);
			var oAmountField = this._oFactory && this._oFactory.oAmountField;

			if (oAmountField) {
				oAmountField.toggleStyleClass("smartFieldPaddingRight", bUomVisible);
			}
		}
	...
	SmartField.prototype.unbindProperty = function(sPropertyName, bSuppressReset) {
		...
		aInnerControls.forEach(function(oInnerControl) {
			...
			var vPropertiesNamesOfInnerControls = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(sControlName, {
	...
	SmartField.prototype.setValue = function(sValue) {
		...
		// control.
		if (oFactory && oFactory.bTextInEditModeSourceValidMetadata && (sValue !== sOldValue) && oControl) {
			...
			var bModelUpdate = isPropertyBeingUpdatedByModel.call(this, "value");

			switch (this.getTextInEditModeSource()) {
				case TextInEditModeSource.NavigationProperty:
					var bDescriptionForValueLoaded = !!this.getModel().getData(oBinding.getBindings()[1].getPath(), this.getBindingContext(), true);
				...
				case TextInEditModeSource.ValueList:
					...
					if (bModelUpdate) {
						...
						// update the URL path name to the OData property of the value list entity where the descriptive text
						// is fetch
						oControl.bindProperty("value", oFactory.getTextArrangementBindingInfo({
	 ...
	 * @returns {any} A <code>Promise</code> or any primitive value
	 * @private
	 */
	SmartField.prototype._checkErrors = function(oControl, mSettings) {
		...
		if (oControl) {
			var aValuesPropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(oControl.getMetadata().getName(), {
				propertyName: "value"
			});
		...
		try {
			...
			if (oBinding.sInternalType && oType) {
				...
				var vParseValueReturn = oType.parseValue(sValue, oBinding.sInternalType, oBinding.getValue()),
	...
	SmartField.prototype.onBeforeValidateValue = function(sValue, mSettings) {
		...
		var oModel = this.getModel(),
			...
			mTextArrangementPaths = ODataControlFactory.getTextArrangementPaths(this.getTextInEditModeSource(), this.getControlFactory().getMetaData()),
			sKeyField = mTextArrangementPaths.keyField,
			sDescriptionField = mTextArrangementPaths.descriptionField;

		oModel.read("/" + mTextArrangementPaths.entitySetName, {
			filters: getValueListFilters(sValue, {
				keyFieldPath: sKeyField,
				descriptionFieldPath: sDescriptionField,
				aFilterFields: mSettings.filterFields
			}),
			urlParameters: {
	...
	SmartField.prototype.onRequestValueListDataSuccess = function(mSettings, oData, oResponse) {
		...
		if (typeof mSettings.success === "function") {
			mSettings.success(oData.results);
		}
	};

	SmartField.prototype.onRequestValueListDataError = function(mSettings, oError) {
		var oControl = this._oControl.edit;
	...
	function getValueListFilters(vValue, oSettings) {
		...
		if (aFilters.length === 1) {
			return [SmartField["getValueList" + capitalize(aFilters[0]) + "Filter"](vValue, oSettings)];
		}
